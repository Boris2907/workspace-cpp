...
void printAll(ds_course::Node *node, char* begin, char* end, char* type) {
    ...
    if (strcmp(begin, "_") == 0 && (strcmp(end, "_") == 0)) {
        ...
        if (comparePlusType(type, end, node->key) >= 0) {
            cout << " (";
            for (int i = 0 ; i < str_length(node->key); i++)
                cout << node->key[i];
            cout << "," << node->badness << ")";
        ...
        if (comparePlusType(type, begin, node->key) <= 0) {
            cout << " (";
            for (int i = 0 ; i < str_length(node->key); i++)
                cout << node->key[i];
            cout << "," << node->badness << ")";
        ...
        if ((comparePlusType(type, end, node->key) == 1 && comparePlusType(type, begin, node->key) == -1) || (comparePlusType(type, end, node->key) == 0 && comparePlusType(type, begin, node->key) == -1) || (comparePlusType(type, end, node->key) == 1 && comparePlusType(type, begin, node->key) == 0)) {
            cout << " (";
            for (int i = 0 ; i < str_length(node->key); i++)
                cout << node->key[i];
            cout << "," << node->badness << ")";
